...
### 汇编语言的书籍
《汇编语言》王爽　著
该书以Intel 8086 CPU（这是一个16位的CPU）为基础，使用MASM编译器，以DOS为平台进行汇编语言的讲解。这是我正在看的书

其他的一些汇编书籍的简介，在[这里](https://baike.baidu.com/item/%E6%B1%87%E7%BC%96%E8%AF%AD%E8%A8%80/61826 "这里")
...
这一步是让我们输入要进行编译的源文件，由于我们源文件是以`.asm`结尾的`2.asm`，所以这里只需要输入`2`即可。如果我们的源文件是以其他后缀结尾的，例如`.txt`，这里就需要输入文件包含后缀名的全称`2.txt`。

输入`2`之后，弹出如下提示：

	Object filename [2.OBJ]

这一步编译器是在询问我们要输出的目标文件的文件名，默认是`2.OBJ`，如果没有自定义文件名的需要，则这里直接回车使用默认的就可以。（在这一步，我们除了可以指定文件名之外，还可以指定文件的输出保存路径）

回车之后弹出提示：

	Source listing [NUL.LIST]

这里让我们指定列表文件的名称。列表文件是编译器在将汇编语言编译成机器语言的过程中产生的中间文件。我们可以直接回车，不让这个列表文件产生。

回车之后弹出提示：

	Cross-reference [NUL.CRF]

这里让我们指定交叉引用文件的名称。交叉引用文件也是编译器在将汇编语言编译成机器语言的过程中产生的中间文件，我们可以直接回车，不让系统产生该文件。

回车之后就看到了这个：

![](http://wx1.sinaimg.cn/mw690/006CBL8rly1ft9ggnk43sj30hy0btt8w.jpg)
...
这表明，该汇编程序没有“警告错误”和“严峻的（必须改正的）错误”。

之后，刷新汇编程序所在的目录可以看到多出一个文件：
